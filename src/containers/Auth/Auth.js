...
export const Auth = (props) => {
  ...
  const onAuth = (email, password, needsSignUp) =>
    dispatch(action.auth(email, password, needsSignUp));
  ...
  if (error) {
    errorMessage = <p className={classes.FormError}>{error.message}</p>;
  }
  ...
  return (
    <section className={classes.Auth}>
      ...
      <div className={classes.SignUpContainer}>
        <h3>{needsSignUp ? "SIGN UP" : "SIGN IN"}</h3>
        {errorMessage}
        <form onSubmit={submitHandler}>
          {form}
          <Button btnType="SubmitSignIn" type="submit">
            SUBMIT
          </Button>
        </form>
        <Button btnType="SignUpSwitch" clicked={switchAuthModeHandler}>
          {needsSignUp
