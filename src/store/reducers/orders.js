...
const reducer = (state = initState, action) => {
  switch (action.type) {
    case actionType.STORE_ORDER_START:
      return storeOrderStart(state, action);
    case actionType.SEND_STORE_ORDER:
      return sendStoreOrder(state, action);
    case actionType.FETCH_ORDER_SUCCESS:
      return fetchOrderSuccess(state, action);
    case actionType.STORE_ORDER_FAILED:
      return storeOrderFailed(state, action);
    default:
      return state;
  }
};
