...
const addItem = (state, action, storeIndex) => {
  return updateObject(state, {
    ...
    cart:
      state.inStockStatus[storeIndex] !== false
        ? // Add item with the same item ID to the cart
          state.cart.concat(
            state.storeInventory.filter((el) => el.itemID === action.itemID),
          )
    ...
    itemInvCount: state.itemInvCount.map((el, ind) =>
      ind === storeIndex && el !== 0 ? el - 1 : el,
    ),
    // IF the count for that item in stock is 0
    inStockStatus:
      state.itemInvCount[storeIndex] - 1 === 0
        ? // Selected item out of stock
          state.inStockStatus.map((elState, ind) =>
            ind === storeIndex ? (elState = false) : elState,
          )
        : // item is in stock
          state.inStockStatus,
...
const removeItem = (state, action, cartIndex, storeIndex) => {
  return updateObject(state, {
    ...
    itemInvCount: state.itemInvCount.map((el, ind) =>
      ind === storeIndex && el !== state.storeInventory[storeIndex].inventory
        ? el + 1
        : el,
    ),
    // Check the stock for that item is at 0
    inStockStatus:
      state.itemInvCount[storeIndex] === 0
        ? // Selected item state is true
          state.inStockStatus.map((elState, ind) =>
            ind === storeIndex ? (elState = true) : elState,
          )
...
const removeAllOfItem = (state, action, cartIndex, storeIndex) => {
  return updateObject(state, {
    ...
    totalPrice:
      state.cart.length !== 0
        ? state.totalPrice -
          state.cart[cartIndex].price *
            (state.storeInventory[storeIndex].inventory -
              state.itemInvCount[storeIndex])
        : 0,
    // Update the inventory count, that item that just got removed, (matching indexes) will be back at original storeInvetory count
    itemInvCount: state.itemInvCount.map((el, ind) =>
      ind === storeIndex && el !== state.storeInventory[storeIndex].inventory
        ? (el = state.storeInventory[storeIndex].inventory)
        : el,
    ),
    // Item should now be back in stock, since it is not in the cart anymore
    inStockStatus: state.inStockStatus.map((elState, ind) =>
      ind === storeIndex ? (elState = true) : elState,
    ),
    // reducedCart: reducedCart,
    ordering: true,
...
const reducer = (state = initState, action) => {
  ...
  if (state.storeInventory) {
    ...
    storeIndex = state.storeInventory.findIndex(
      (el) => el.itemID === action.itemID,
    );
  ...
  switch (action.type) {
    case actionType.ADD_ITEM:
      return addItem(state, action, storeIndex);
    case actionType.REMOVE_ITEM:
      return removeItem(state, action, cartIndex, storeIndex);
    case actionType.REMOVE_ALL_OF_ITEM_TYPE:
      return removeAllOfItem(state, action, cartIndex, storeIndex);
    case actionType.FETCH_STORE_INVENTORY_FAILED:
      return fetchStoreInventoryFailed(state, action);
    case actionType.SET_STORE_INVENTORY:
      return setStoreInventory(state, action);
    default:
      return state;
  }
};
